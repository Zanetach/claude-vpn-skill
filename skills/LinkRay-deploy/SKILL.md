...
- For Mihomo clients, include `meta-rules-dat` `rule-providers` and visible strategy groups when the user wants routing rules. At minimum expose `PROXY`, `AUTO`, and service groups such as `OpenAI`, `GitHub`, `Google`, `YouTube`, `Netflix`, `Telegram`, and `Global`; route each `RULE-SET` to the matching group. Keep the last rule as `MATCH,Global` or `MATCH,PROXY` so rule-provider download failures do not leave traffic unhandled.
...
| Returning rules but no visible strategy groups | Add `OpenAI`, `GitHub`, `Google`, `YouTube`, `Netflix`, `Telegram`, and `Global` groups, then map `RULE-SET`s to those groups |
