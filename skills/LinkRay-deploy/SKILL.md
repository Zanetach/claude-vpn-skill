...
- For Mihomo clients, include `meta-rules-dat` `rule-providers` in the adapted profile when the user wants routing rules. Keep the last rule as `MATCH,PROXY` so rule-provider download failures do not leave traffic unhandled.
